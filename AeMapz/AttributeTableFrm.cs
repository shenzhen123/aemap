...
namespace AeMapz
{
    public partial class AttributeTableFrm : Form
    {
        ...
        /// </summary>
        /// <param name="fiedlType"></param>
        /// <returns></returns>
        public static string ParseFieldType(esriFieldType fiedlType)
        {
            switch (fiedlType)
            {
        ...
        public static DataTable CreateDataTable(ILayer pLayer, string tableName)
        {
            ...
            while(pRow != null)
            {
                ...
                {
                    //如果字段类型为esriFieldTypeGeopetry，则根据图层类型设置字段
                    if(pRow.Fields.get_Field(i).Type == esriFieldType.esriFieldTypeGeometry)
                    {
                        pDataRow[i] = shapeType;
                    }
                    //当图层类型为Anotation时，要素类中会有esriFieldTypeBlob类型的数据，
                    //其存储的时标注内容，如此情况需将对应的字段值设置为Element
